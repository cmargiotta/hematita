...
use self::super::{
    ...
    vm::{
        ...
        BinaryOperation, Chunk, OpCode, UnaryOperation,
    },
};
    ...
    fn register(self, compiler: &mut Generator) -> usize {
        match self {
            ...
            Self::Register(register, tuple) => {
                if tuple {
                    compiler.unwrap_tuple(register)
                }
                register
            }
        }
...
impl Generator {
    ...
    fn compile_known(&mut self, value: impl Into<KnownValue>) -> usize {
        ...
        if let Some(register) = self
            .registers
            .iter()
            .position(|known| known.as_ref().map(|known| known == &value).unwrap_or(false))
        {
            return register;
        }
    ...
    fn finish(self) -> Chunk {
        ...
        let up_values = (0..up_values
            .values()
            .filter_map(|&(a, b)| (!b).then(|| a))
            .max()
            .map(|v| v + 1)
            .unwrap_or(0))
            .map(|a| (a, false))
            ...
            true => {
                up_value
                    + self
                        .up_values
                        .values()
                        .filter_map(|(up_value, use_up_value)| (!use_up_value).then(|| *up_value))
                        .max()
                        .unwrap_or(0)
            }
            false => up_value,
    ...
    fn compile(&mut self, block: &Block) {
        ...
        loop {
            if current_statement >= block.len() {
                return;
            }
            ...
            match &block[current_statement] {
                ...
                Statement::GenericFor {
                    ...
                    self.variables_to_registers
                        .insert(variable.clone(), destination);
                    self.compile(r#do);
                ...
                Statement::Assign { variables, values } => {
                    ...
                    for target in variables {
                        match (target, value(self)) {
                            (
                                ...
                                (Some(_), _, _) => names
                                    .push((identifier.clone(), CompileResult::Evaluated(value))),
                                ...
                                (None, None, None) => {
                                    let register = self.compile_known(value);
                                    self.opcode(OpCode::SaveGlobal {
                                        register,
                                        global: identifier.clone(),
                                    });
                                    // FIXME: Requires GATs
                    ...
                    names.into_iter().for_each(|(name, value)| match value {
                    ...
                    for name in variables {
                        match value(self) {
                            CompileResult::Evaluated(value) => {
                                ...
                                // it's known.
                                names.push((name.clone(), CompileResult::Evaluated(value)));
                            }
                ...
                Statement::Function {
                    ...
                    if !name_rest.is_empty() {
                        let indexee = self
                            .compile_expression(&Expression::Identifier(name_first.clone()))
                            .register(self);
                        ...
                        self.opcode(OpCode::SaveGlobal {
                            register,
                            global: name_first.clone(),
                        });
                    }
                }

                Statement::Method {
                    class: (class_first, class_rest),
                    name,
                    arguments,
                    body,
                } => {
                    ...
                    let indexee = self
                        .compile_expression(&Expression::Identifier(class_first.clone()))
                        .register(self);
    ...
    fn compile_expression(&mut self, expression: &Expression) -> CompileResult {
        match expression {
            // Identifier
            Expression::Identifier(identifier) => match self.evaluated_variables.get(identifier) {
                ...
                None => match self.variables_to_registers.get(identifier) {
                    ...
                    None => match self.up_values.get(identifier) {
                        Some(&up_value) => {
                            let register = self.register();
                            self.opcode(OpCode::LoadUpValue {
                                up_value: self.up_value_id(up_value),
                                register,
                            });
    ...
    where
        T: Iterator<Item = &'i dyn CompileOrCompiled>,
    {
        ...
        loop {
            match (
                tuple.next().map(|compileable| compileable.compile(self)),
                tuple.peek().is_some(),
            ) {
                // If the last argument is a tuple, and it's the *only* argument, use
                // it as the tuple.
                (Some(CompileResult::Register(register, true)), false) if index == 0 => {
                    break register
                }
    ...
    fn compile_call(&mut self, function: &Expression, arguments: &[Expression]) -> usize {
        let function = self.compile_expression(function).register(self);
        let arguments = self.compile_tuple(arguments.iter().map(CompileOrCompiled::r#dyn));
        let destination = self.register();
        self.opcode(OpCode::Call {
            function,
            arguments,
    ...
    fn compile_method_call(
        ...
        let arguments = self.compile_tuple(
            once(class.r#dyn()).chain(arguments.iter().map(CompileOrCompiled::r#dyn)),
        );
    ...
    fn prepare_side_effects(&mut self) {
        ...
        let evaluated = self
            ...
            .collect::<Vec<_>>();
        evaluated.into_iter().for_each(|(name, value)| {
            let register = self.compile_known(value);
...
where
    I: ExactSizeIterator<Item = CompileResult> + 'i,
{
    enum State<I>
    where
        I: ExactSizeIterator<Item = CompileResult>,
    {
        Values(I),
        Call {
            tuple: usize,
