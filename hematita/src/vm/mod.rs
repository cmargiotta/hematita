...
use self::{
    ...
    constant::Constant,
    value::{Function, IntoNillable, MaybeUpValue, Nil, Nillable, NonNil, Table, Value},
};
use std::{
    cmp::max,
    collections::HashMap,
...
impl<'n> VirtualMachine<'n> {
    ...
    pub fn execute(
        ...
        let registers = vec![Default::default(); function.chunk.registers].into_boxed_slice();
...
impl<'v, 'f, 'n> StackFrame<'v, 'f, 'n> {
    ...
    fn reference<'s>(&self, reference: impl Into<Reference<'s>>) -> Nillable<'n> {
        match reference.into() {
            ...
            Reference::Local(id) => match &self.registers[id] {
                ...
                MaybeUpValue::UpValue(value) => value.lock().expect("poison error").clone(),
            },
        }
    }
    ...
    fn write_reference<'s>(&mut self, reference: impl Into<Reference<'s>>, value: Nillable<'n>) {
        match reference.into() {
            ...
            Reference::Local(id) => match &mut self.registers[id] {
                MaybeUpValue::Normal(destination) => *destination = value,
    ...
    /// ------
    /// Panics if any encountered lock is poisoned.
    fn meta_method(&self, object: &Nillable<'n>, method: &str) -> Nillable<'n> {
        match object {
            ...
            NonNil(Value::Table(table)) => match &*table.metatable.lock().expect("poison error") {
                Some(meta) => {
                    let meta = meta.data.lock().expect("poison error");
                    meta.get(&Value::new_string(method)).nillable()
                }
    ...
    fn call(
        ...
        match function {
            ...
            NonNil(Value::NativeFunction(function)) => function(arguments, self.virtual_machine),
            ...
            function => Err(format!("attempt to call a {} value", function.type_name())),
        }
    }
            ...
            match chunk.opcodes[current_opcode] {
                OpCode::Call {
                    ...
                    let arguments = match self.reference(arguments) {
                        ...
                        value => {
                            break Err(format!(
                                "attempt to call a function with a {} value",
                                value.type_name()
                            ))
                        }
                    };
                ...
                OpCode::IndexWrite {
                    ...
                    indexee => {
                        break Err(format!("attempt to index a {} value", indexee.type_name()))
                    }
                },

                OpCode::Create { destination, .. } => {
                    self.write_reference(destination, NonNil(Value::Table(Table::default().arc())));
                }
                ...
                OpCode::BinaryOperation {
                    ...
                    destination,
                    match (self.reference(left), self.reference(right), operation) {
                        // Arithmetic
                ...
                OpCode::UnaryOperation {
                    ...
                    destination,
                    match (self.reference(operand), operation) {
                        // Arithmetic
                ...
                OpCode::Return { result } => match self.registers.get(result) {
                    ...
                    Some(&MaybeUpValue::UpValue(ref value)) => match &*value
                        .lock()
                        .expect("poison error")
                    {
    ...
    fn index_read(
        ...
        match match match (&indexee, &index) {
            ...
            IndexOperation::Value(NonNil(value)) => IndexOperation::Value(NonNil(value)),
        } {
            IndexOperation::Value(value) => self.write_reference(destination, value.clone()),
            IndexOperation::NilIndex => return Err("table index is nil".to_owned()),
            IndexOperation::NonTable => {
                return Err(format!("attempt to index a {} value", indexee.type_name()))
            }
        }
...
impl<'s> Display for OpCode<'s> {
    fn fmt(&self, f: &mut Formatter) -> FMTResult {
        match self {
            ...
            Self::Create { destination } => write!(f, "crt {};", destination),
            Self::BinaryOperation {
                ...
                // Arithmetic
                BinaryOperation::Add => write!(f, "aadd {}, {}, {};", left, right, destination),
                BinaryOperation::Subtract => {
                ...
                BinaryOperation::Divide => write!(f, "adiv {}, {}, {};", left, right, destination),
                ...
                BinaryOperation::Modulo => write!(f, "amod {}, {}, {};", left, right, destination),
                ...
                // Relational
                BinaryOperation::Equal => write!(f, "req {}, {}, {};", left, right, destination),
                BinaryOperation::NotEqual => write!(f, "rne {}, {}, {};", left, right, destination),
                BinaryOperation::LessThan => write!(f, "rlt {}, {}, {};", left, right, destination),
                ...
                // Other
                BinaryOperation::Concat => write!(f, "cat {}, {}, {};", left, right, destination),
            },
            Self::UnaryOperation {
                operand,
                destination,
                ...
                // Arithmetic
                UnaryOperation::Negate => write!(f, "aneg {}, {};", operand, destination),

                // Bitwise
                UnaryOperation::BitwiseNot => write!(f, "bnot {}, {};", operand, destination),

                // Logical
                UnaryOperation::LogicalNot => write!(f, "not {}, {};", operand, destination),

                // Other
                UnaryOperation::Length => write!(f, "len {}, {};", operand, destination),
            },
            Self::Jump {
                operation,
                r#if: None,
            ...
            Self::Return { result } => write!(f, "ret {};", result),
            Self::ReAssign { actor, destination } => write!(f, "reas {}, {};", actor, destination),
